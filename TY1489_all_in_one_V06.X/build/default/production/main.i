...
volatile uint8_t T10MS_CNT1 = 0;
_Bool fUnlock = 0;
_Bool fClear = 0;
_Bool fReceive_failed = 0;

_Bool fDRL_Gradually = 0;

_Bool fPOS_Gradually = 0;
_Bool fPOS_Gradually2 = 0;
_Bool fLamp_RL_Side = 0;
_Bool fLamp_Conf = 0;
_Bool fLoPe_DRL_In_use = 0;
...
void Exception_handling2(void) {
  if ((data_buf[2] == 2) && (data_buf[10] == 0xB4)) {
    fClear = 1;
    P2_OFF();
    LoBeam_OFF();
    HiBeam_OFF();
    POS_OFF();
    if (fPOS_Gradually2 == 0) {
      (INTCONbits.GIE = 0);
      POS_Slow_extinguishing();
      (INTCONbits.GIE = 1);
    }
    T10MS_CNT = 0;
    T10MS_CNT1 = 0;
...
void HiBeam_ON(void) { do { LATAbits.LATA0 = 0; } while(0); }

void HiBeam_OFF(void) { do { LATAbits.LATA0 = 1; } while(0); }
...
void DRL_OFF(void) { CCPR2L = 0; }
void POS_ON(void) { CCPR2L = 14; }
void POS_OFF(void) { CCPR2L = 0; }
void P2_ON(void) { do { LATBbits.LATB7 = 1; } while(0); }
void P2_OFF(void) { do { LATBbits.LATB7 = 0; } while(0); }
...
void POS_Gradually(void) {
  for (uint8_t i = 0; i < 49; i++) {
    CCPR2L = i + 12;
    _delay((unsigned long)((41)*(32000000/4000.0)));
  }
  CCPR2L = 60;
  fPOS_Gradually = 1;
}
...
# 774 "main.c"
  ...
  while (1) {
    ...
    Exception_handling();
    Exception_handling2();
    if ((fReceive_failed == 0)) {
      if ((fUnlock == 1)||(fClear == 1)) {
